...
public class IntegratorRKN_2DArray_Sloshing {
	SecondDerivative_Sloshing secondDerivative;
    double t0, y0, dy0, h;
    double d2y0;
    double [] previousX;

    public IntegratorRKN_2DArray_Sloshing (SecondDerivative_Sloshing secondDerivative, double t0, 
    ...
    public IntegratorRKN_2DArray_Sloshing step(double t, double y, double dy,  double d2y, double[] previousX)
    {
       double h2 = h * h; 
    ...
    public IntegratorRKN_2DArray_Sloshing stepByHalfM(double t, double y, double dy,  double d2y, double[] previousX)
    {
       double h2 = h * h; 

       double k1 = d2y0;
       double k2 = secondDerivative.getValueByHalfM(
           t0 + h/2, y0 + h/2 * dy0 + h2/8 * k1, dy0 + h/2 * k1, previousX);
       double k3 = secondDerivative.getValueByHalfM(
           t0 + h/2, y0 + h/2 * dy0 + h2/8 * k2, dy0 + h/2 * k2, previousX);
       double k4 = secondDerivative.getValueByHalfM(
           t0 + h,   y0 +   h * dy0 + h2/2 * k3, dy0 +   h * k3, previousX);

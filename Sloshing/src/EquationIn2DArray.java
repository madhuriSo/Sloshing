...
public class EquationIn2DArray implements SecondDerivative_Sloshing {
    ...
    public double getFeByHalfM(double t){
        return -m/2* OMEGA * 2 *(Math.sin(OMEGA*t));
    }
	...
	@Override
	public double getValueByHalfM(double t, double y, double dy, double[] previousX) {
		double f = 0 ,df1 = 0,df2=0,fe;
        double x0=previousX[0],x0_dot=previousX[1],x0_ddot=previousX[2];
        double x1=previousX[3],x1_dot=previousX[4],x1_ddot=previousX[5];
        double x2=previousX[6],x2_dot=previousX[7],x2_ddot=previousX[8];
